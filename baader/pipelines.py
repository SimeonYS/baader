...
class BaaderPipeline:

    # Database setup
    conn = sqlite3.connect('baader.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `baader`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM baader WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO baader (date, title, link, content)"
